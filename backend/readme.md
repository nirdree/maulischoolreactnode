...
enquirySchema.pre('save', async function (next) {
  if (!this.enquiryId) {
    ...
    this.enquiryId = `ENQ-${year}-${String(count + 1).padStart(4, '0')}`;
  }
  next();
});
...
marksSchema.pre('save', function (next) {
  if (!this.isAbsent) {
    // Grade is calculated by caller; fallback logic here
    if (!this.grade) {
      const pct = this.marksObtained;  // caller passes grade anyway
      this.grade = pct >= 90 ? 'A+' : pct >= 80 ? 'A' : pct >= 70 ? 'B+' :
...
// ── GET /api/marks?studentId=&classId=&academicYear= ─────────
router.get('/', async (req, res) => {
  try {
